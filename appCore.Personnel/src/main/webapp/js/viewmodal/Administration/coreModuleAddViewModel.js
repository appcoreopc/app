var CoreModuleInfoViewModel = function (mode, globalViewModel) {
    ...
    if (self.globalViewModel.applicationScopeType() != coreApplicationTypeCoreModuleType) {
        throw "Application Type is not coreModule.";
    }
    ...
    function initializeApplication() {
        ...
        var input = { "id":coreCoreModulePage, "roleId":self.globalViewModel.employeeRole() };
        ...
        if (self.mode() == coreModeEdit) {
            ...
            helper.getCoreModule(entityData, getEntityGetDataCallback);
        }
    }
    ...
    self.closeAddControl = function () {
        $("#accordianCoreModule").accordion({collapsible:true, active:false});
    }
    ...
    self.updateInfoData = function (data) {
        ...
        var helper = new CompanyHelper();
        helper.saveOrUpdateCoreModuleInfo(entityData, updateDataSuccessCallback);
        self.editData("");
    }
    ...
    self.addInfo = function (data) {
        ...
        if (self.addInfoCategory() == "" || self.addInfoCategory() == undefined) {
            $("#coreModuleCategoryAdd").validationEngine('validate');
            validationStatusOk = false;
        }

        if (self.addInfoType() == "" || self.addInfoType() == undefined) {
            $("#coreModuleTypeAdd").validationEngine('validate');
            validationStatusOk = false;
        }
        ...
        if (validationStatusOk) {
            ...
            var helper = new CompanyHelper();
            helper.saveOrUpdateCoreModuleInfo(entityInfoData, addDataSuccessCallback);
        }
    }
    ...
    self.templateToUse = function (item) {
        switch (self.mode()) {
            case 1:
            case 2:
                return "CoreModuleEntityAddTemplate";
            default:
                return "CoreModuleEntityAddTemplate";
        }
        return "CoreModuleEntityAddTemplate";
    ...
    function deleteInfoConfirmCallBack(userResponse, data, globalViewModel, codeType) {
        ...
        if (userResponse == true) {
            ...
            var result = helper.deleteCoreModuleInfo(entityData, data, deleteCompleteCallback);
        }
    }
    ...
    self.updateData = function (data) {
        ...
        var isValid = $("#" + "coreModuleForm").validationEngine('validate');
        ...
        if (self.mode() == coreModeEdit)
            coreModule.nid = self.nid();

        coreModule.companyId = self.globalViewModel.companyId();
        coreModule.name = self.name();
        coreModule.description = self.description();

        var helper = new CompanyHelper();
        helper.saveUpdateCoreModule(coreModule, saveOrUpdateStatus)
    }

    function saveOrUpdateStatus(result) {
        if (result.messageCode == 0) {
            preparePageForLoading("coreModuleListPage.jsp");
        }
    }
