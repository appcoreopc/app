var PayrollTableValueInfoViewModel = function (mode, globalViewModel) {
    ...
    var self = this;
	 ...
	 self.directValue = ko.observable();
    ...
    if (self.globalViewModel.applicationScopeType() != coreApplicationTypePayrollTableValueType) {
        throw "Application Type is not payrollTableValue.";
    }
    ...
    function initializeApplication() {
        ...
        var input = { "id":corePayrollTableValuePage, "roleId":self.globalViewModel.employeeRole() };
        ...
        if (self.mode() == coreModeEdit) {
            ...
            helper.getPayrollTableValue(entityData, getEntityGetDataCallback);
        }
    }
    ...
    function getEntityGetDataCallback(data) {
        ...
        if (data != null) {
				    ...
				    self.directValue(data.directValue);
    ...
    self.updateInfoData = function (data) {
        ...
        var helper = new CompanyHelper();
        helper.saveOrUpdatePayrollTableValueInfo(entityData, updateDataSuccessCallback);
        self.editData("");
    }
    ...
    self.addInfo = function (data) {
        ...
        if (self.addInfoCategory() == "" || self.addInfoCategory() == undefined) {
            $("#payrollTableValueCategoryAdd").validationEngine('validate');
            validationStatusOk = false;
        }

        if (self.addInfoType() == "" || self.addInfoType() == undefined) {
            $("#payrollTableValueTypeAdd").validationEngine('validate');
            validationStatusOk = false;
        }
        ...
        if (validationStatusOk) {
            ...
            var helper = new CompanyHelper();
            helper.saveOrUpdatePayrollTableValueInfo(entityInfoData, addDataSuccessCallback);
        }
    }
    ...
    self.templateToUse = function (item) {
        switch (self.mode()) {
            case 1:
            case 2:
                return "PayrollTableValueEntityAddTemplate";
            default:
                return "PayrollTableValueEntityAddTemplate";
        }
        return "PayrollTableValueEntityAddTemplate";
    ...
    function deleteInfoConfirmCallBack(userResponse, data, globalViewModel, codeType) {
        ...
        if (userResponse == true) {
            ...
            var result = helper.deletePayrollTableValueInfo(entityData, data, deleteCompleteCallback);
        }
    }
    ...
    self.updateData = function (data) {
        ...
        var isValid = $("#" + "payrollTableValueForm").validationEngine('validate');
        ...
        var payrollTableValue = {};
        if (self.mode() == coreModeEdit)
            payrollTableValue.nid = self.nid();

														payrollTableValue.nid = self.nid();
														payrollTableValue.start = self.start();
														payrollTableValue.end = self.end();
														payrollTableValue.setupType = self.setupType();
														payrollTableValue.typeRefId = self.typeRefId();
														payrollTableValue.directValue = self.directValue();
														payrollTableValue.description = self.description();
														payrollTableValue.application = self.application();
														payrollTableValue.lastUpdate = self.lastUpdate();
														payrollTableValue.companyId = self.companyId();
						

        var helper = new CompanyHelper();
        helper.saveUpdatePayrollTableValue(payrollTableValue, saveOrUpdateStatus)
    }

    function saveOrUpdateStatus(result) {
        if (result.messageCode == 0) {
            preparePageForLoading("payrollTableValueListPage.jsp");
        }
    }
